	...
	export default {
		...
		methods: {
			...
			goTo(item){
				...
				// return
				uni.navigateTo({
					url:`../../question/boostExpert/settleIn/cooperation/cooperation?index=${this.index}&cat_id=${item.cat_id}&cat_name=${item.cat_name}`
					// url:`./release?cat_id=${item.cat_id}&cat_name=${item.cat_name}`
				})
			},
			close(){
				uni.navigateBack({
