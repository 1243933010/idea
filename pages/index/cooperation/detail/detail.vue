	...
	<view>
		<view class="context">
			...
			<view class="item" >
				<!-- <view class="item-user">
					...
					<view class="delete" v-if="obj.user_id==userInfo.user_id">
				...
				<view class="item-form">
					<view class="inp">
						<text>姓名:</text>
						<input type="text" value="" v-model="obj.name" />
					</view>
					<view class="inp">
						<text>联系方式:</text>
						<input type="text" value="" v-model="obj.phone" />
					</view>
				</view>
				<view class="item-con">
					<view class="item-con-border">
					...
					<view class="item-con-con">
						...
						<view class="item-con-con-img">
							<image v-for="(item,index) in obj.url_data" :key="index"  v-if="item.type===1" :src="item.url" mode=""></image>
						</view>
					</view>
				</view>
		...
		<view class="comment">
			<view class="comment-title">
				<text>评论（{{obj.comment.length}}）</text>
			</view>
			<view class="comment-item" v-if="obj.comment.length!==0">
				<view class="item" v-for="(item,index) in obj.comment" :key="index" >
					<view class="item-flex" @longpress="longpress(item)">
						<view class="icon">
							<image :src="item.user_photo" mode=""></image>
						</view>
						<view class="context">
							...
							<view class="bottom">
								...
								<view class="right">
									...
									<view class="zan"  @click="setZan(item,'2')">
										...
										<image v-if="item.is_click=='2'"  style="width: 45rpx;" src="../../../../static/s_zan_y.png" mode="widthFix"></image>
					...
					<view class="huifu" v-if="item.children.length!==0">
						<view class="huifu-item" v-for="(v,i) in item.children" :key="i">
							<view class="huifu-item-uer" style="margin-bottom: 15rpx;">
								<image :src="v.user_photo" mode=""></image>
								<text>{{v.alias}}</text>
								
							</view>
							<view class="huifu-item-con" style="margin-left: 15rpx;margin-bottom: 10rpx;">
								<text>{{v.content}}</text>
							</view>
							<view class="huifu-item-time">
								<text class="time">{{v.create_time}}</text>
								<view class="zan">
									<image style="margin-right:26rpx;" v-if="v.is_delete=='1'"  @click="shan(v)" src="../../../../static/idea/delete.png" mode="widthFix"></image>
		...
		<view class="fixed">
			<view class="fixed-con">
				<view class="inp">
					<input type="text" v-model="content" placeholder-style="color:#262626;font-size:24rpx;text-indent:15rpx;" confirm-type="send" @confirm="fabu" placeholder="写评论..." />
				</view>
				<!-- <view class="fabu" @click="fabu"><text>发布</text></view> -->
				<view class="img">
					<!-- <image class="collection" src="../../static/idea/xing.png" mode="widthFix"></image> -->
					<image v-if="obj.is_collect=='1'" @click="favorites(0)"  style="width: 40rpx;" class="share" src="../../../../static/no_flower.png" mode="widthFix"></image>
					<image v-if="obj.is_collect=='2'"  @click="favorites(1)"  style="width: 40rpx;" class="share" src="../../../../static/is_flower.png" mode="widthFix"></image>
					<image v-if="obj.is_click=='1'" @click="setZan(obj,'1')"  style="width: 45rpx;" class="share" src="../../../../static/s_zan_n.png" mode="widthFix"></image>
					<image v-if="obj.is_click=='2'" @click="setZan(obj,'2')"  style="width: 45rpx;" class="share" src="../../../../static/s_zan_y.png" mode="widthFix"></image>
				</view>
			</view>
		</view>
		...
		<view class="popup" v-show="showFlog" @click="OnNavigationBarButtonTap">
			<view class="bk" >
				<view class="bk-con" @click.stop="" :style="{height:obj.is_delete=='0'?'155rpx':'205rpx'}">
					<view class="" @click.stop="share">
						<image src="../../../../static/idea/s_share.png" mode="widthFix"></image>
						<text>分享</text></view>
					<view class="" @click.stop="shanchu"  v-if="obj.is_delete=='1'">
...
<!-- 	<view class="fixed1" v-show="flog" @click.stop="hideFlog">
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			this.getMsg(data.cooperate_id,data.index);
			this.cooperate_id = data.cooperate_id;
		...
		methods:{
			...
			async getMsg(a,b){
				...
				let obj = {cooperate_id:a,login_user_id:userInfo.user_id,type:b+1};
				let data = await this.$request('/api/v1/service_by_demand_detail',obj);
				console.log(data);
			...
			async share(){
				let obj = {
					cooperate_id:this.obj.cooperate_id,
					type:this.index+1
				}
				let data = await this.$request('/api/v1/share_servicedemand_detail',obj)
				console.log(data);
				this.$toast(data.data.message);
				if(data.data.status==='success'){
			...
			async jubao(){
				...
				let data = await this.$request('/api/v1/report_servicedemand_detail',obj)
				console.log(data);
				this.$toast(data.data.message);
				if(data.data.status==='success'){
					this.$refs.popup.close();
			...
			async fabu(){
				...
				let data = await this.$request('/api/v1/release_servicedemand_comment',obj)
				console.log(data);
				this.$toast(data.data.message);
				if(data.data.status==='success'){
					this.content = '';
					this.getMsg(this.obj.cooperate_id,this.index)
					// this.$refs.popup.close();
				}
			},
			pinglun(item){
			...
			async setZan(obj,type){
				...
				let res = {type}
				if(type==='1'){
					res.cooperate_id=obj.cooperate_id;
				}
				if(type==='2'){
					res.comment_id=obj.comment_id;
				}
				let data = await this.$request('/api/v1/click_servicedemand_operate',res);
				console.log(data);
				this.$toast(data.data.message);
				if(data.data.status==='success'){
					this.getMsg(this.obj.cooperate_id,this.index);
			...
			async shan(item){
				console.log(item)
				let userInfo = uni.getStorageSync('userInfo');
				let obj = {comment_id:item.comment_id,login_user_id:userInfo.user_id};
				let data = await this.$request('/api/v1/delete_servicedemand_comment',obj);
				this.$toast(data.data.message);
				if(data.data.status==='success'){
					this.getMsg(this.obj.cooperate_id,this.index);
				}
			...
			async favorites(type){
				console.log(type)
				let obj = {
					type:this.index+1,
					cooperate_id:this.obj.cooperate_id
				}
				let data = await this.$request('/api/v1/collect_servicedemand_operate',obj)
				this.$toast(data.data.message);
				if(data.data.status==='success'){
					this.getMsg(this.obj.cooperate_id,this.index);
				}
			...
			async shanchu(){
				...
				let data = await this.$request('/api/v1/delete_servicedemand',obj)
				this.$toast(data.data.message);
				if(data.data.status==='success'){
					setTimeout(()=>{
						uni.navigateBack({
